...
//new functions
bool hasMinusF(string fullInput);
string getFullFiles(int argc, char * argv[]);

string programName;
int main(int argc, char *argv[]){
    //create commandArguments string
    string commandArguments;
    for(int i = 0; i < argc; i++){
        commandArguments += argv[i];
        commandArguments += ' ';
    }
    //get rid of unused argc warning and test for correct -arguments
    string fileContent;
    if(argc >= 1){
            bool hasFiles = hasMinusF(commandArguments);
            if(hasFiles == true){
            fileContent = getFullFiles(argc, argv);
            cout << "file contents are " << fileContent;
            }
...
bool hasMinusF(string fullInput){
bool hasFiles = false;
int argLength = fullInput.length();
string argSpecifyBuffer;
//check if it has minus f
int j = 1;
for(int i = 0 ; i < argLength; i++){
    if(fullInput[i] != '-'){
        ...
        while(fullInput[i+j] != ' '){
            argSpecifyBuffer = fullInput[i+j];
        }
    }
}

//check argSpecifyBuffer for correct input and if f is it has files
for(int i = 0 ; i < argSpecifyBuffer.length(); i++){
    if((argSpecifyBuffer[i] != 'f') && (argSpecifyBuffer[i] != 'i') && (argSpecifyBuffer[i] != 'v')){
        cerr << fullInput << " has an incorrect -<arg> character, in program " << programName << " system exiting." << endl;
        exit(1);
    }
    if(argSpecifyBuffer[i] == 'f'){
        hasFiles = true;
        //read in the files
        //get the file names
    }
...
string getFullFiles(int argc, char * argv[]){
    ...
    string fullInput;
    while (inFile >> fullInput) {
        fullInput += " ";
    }

    inFile.close();
    fileContents = fullInput;
    return fileContents;
}
