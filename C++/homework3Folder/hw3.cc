...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "+vif:")) != -1) {
		...
		case 'i':
			...
			break;
		default: /* '?' */
			cerr << "Incorrect format specifier was specified. In program "
					<< programName << " you typed " << argv[optind] << endl;
			exit (EXIT_FAILURE);
		}
	} // end while loop to find command line arguments

	if ((outputType == 'i') && (replaceString == true)) {
		cerr
				<< "Both -i and -f were specified which are mutually exclusive. You typed: "
				<< userTyped(argc, argv);
		cerr << "In program named: " << programName << endl;
		exit(1);
	}

	//check the commandLine for files to open
	vector < string > files = canFileOpen(argc, argv, optind);
	...
	if ((files.size() == 0)) {
		...
		while (getline(cin, line)) {
			...
			if (opCount == 0) {
				printTime(outputType, replaceString, line, argc, argv,
						stringToReplace);
			} else {
				string theDate;
				cin.getline(fullString, maxLength, '\n');
				printTime(outputType, replaceString, fullString, argc, argv,
						stringToReplace);
				//call print method
			}
		}
	...
	if (files.size() > 0) {
		...
		for (size_t i = 0; i < files.size(); i++) {
			...
			for (size_t j = 0; j < fileContents.size(); j++) {
				...
				printTime(outputType, replaceString, fileContents[j], argc,
						argv, stringToReplace);
			}
