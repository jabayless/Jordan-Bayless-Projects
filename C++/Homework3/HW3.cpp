...
void printTime(char outputType, bool stringReplace, string date,
		int argc, char *argv[], char * toReplace);
            ...
            if(fullInput.size() != 0){
               ...
               // cout << "time struct is set\n";
                ...
                //ISO formatted output
                    ...
                    strftime(printString, 100, "%m-%d", fullTime);
                    cout << printLine << "-" << printString << endl;
                    return;
                   // toReplace = {'%','Y','-','%','m','-','%','d'};
...
vector<int> isValidDashDate(string date) {
	...
	if ((!checkAllDigits(monthDays)) || (!checkAllDigits(years)) || (!checkAllDigits(months))
        || (dashCount != 2) || (!allValidChars) || (date.length() != 10)) {
		cerr << "Date is not of the correct format. for input " << date << " in program " << programName
				<< " system exiting" << endl;
		exit(1);
	}
